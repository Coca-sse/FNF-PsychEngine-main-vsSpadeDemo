...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('shit without the guy'));
		...
		var balls:FlxSprite;
		balls = new FlxSprite(-80).loadGraphic(Paths.image('shit without bg'));
		balls.antialiasing = true;
		balls.x += 110;
		add(balls);

		balls.x += 900;
			FlxTween.tween(balls, {x: balls.x - 900}, 1.5, {
		...
		/*if(optionShit.length > 6) {
			...
			var menuItem:FlxSprite = new FlxSprite(100,270);
			...
			//FreePlay Mode
			var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
			var menuItem:FlxSprite = new FlxSprite(100,365);
			menuItem.scale.x = scale;
			menuItem.scale.y = scale;
			...
			//Credits
			var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
			var menuItem:FlxSprite = new FlxSprite(100,470);
			menuItem.scale.x = scale;
			menuItem.scale.y = scale;
		...
		var bg:FlxSprite = new FlxSprite(165, 10).loadGraphic(Paths.image("SpadeLogo"));
                add(bg);
	...
	function changeItem(huh:Int = 0)
	{
		...
		menuItems.forEach(function(spr:FlxSprite)
		{
			...
			if (spr.ID == curSelected) //when selected
				{
					if (spr.ID == 0) {
